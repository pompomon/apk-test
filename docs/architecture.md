...
- **`AdventureConfig`** (`game/core/AdventureConfig.kt`): per-difficulty rules — Easy 5 lives / 5 mazes / +1 NPC per maze level, Medium 3 / 7 / +2, Hard 1 / 9 / +3. `npcCountForMaze(idx)` = `idx + extraNpcsPerMaze`. Unknown presets fall back to Medium rules.
