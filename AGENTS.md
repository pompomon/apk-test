...
7. **Instrumented test assertions about overlays use the `@VisibleForTesting` snapshot hooks** (`GameMenuPopover.textSnapshotForTesting()`, `MainActivity.menuPopoverTextSnapshotForTesting()`, `MainActivity.isMenuPopoverShowingForTesting()`). Do **not** use Espresso `isPlatformPopup()` / focus-root matchers — they are flaky on CI emulators. (PR #11, PR #12)
